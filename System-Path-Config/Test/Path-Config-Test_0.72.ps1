...
Assert-Contains $Source 'var VERSION = "0\.72";' "HTA internal version is not 0.72."
Assert-Contains $Source '(?s)if \(state\.mode === "Apply" && state\.activeUiTab > 0\).*?onclick="applyCurrentProgram\(\)">APPLY .*?</button>.*?<button class="button button-primary .*?">MODE: ' "Current-tab Apply is not immediately before the mode button in the Apply-mode top bar."
