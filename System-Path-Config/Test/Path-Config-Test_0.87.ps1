...
Assert-Contains $Source 'var VERSION = "0\.87";' "HTA internal version is not 0.87."
Assert-Contains $Source 'if \(!pinnedTreeHasShortcut\[i\] && !start11GroupContainsSignature' "Start11 Apply does not preserve a shortcut moved into a custom group in either registry tree."
$CommonApplyIndex = $Source.IndexOf('onclick="return runApplyAction(applyPaths)">APPLY PATHS</button>')
$ProgramApplyIndex = $Source.IndexOf('onclick="return runApplyAction(applyCurrentProgram)">APPLY ')
$ModeApplyIndex = $Source.IndexOf('onclick="toggleMode()">MODE: ')
if ($CommonApplyIndex -lt 0 -or $ProgramApplyIndex -lt 0 -or $ModeApplyIndex -lt 0 -or
        $CommonApplyIndex -gt $ModeApplyIndex -or $ProgramApplyIndex -gt $ModeApplyIndex) {
    $Failures.Add("Common and program Apply actions are not before the mode button in the top bar.")
}
Assert-NotContains $Source 'footer-actions[^
]*applyPaths' "Apply Paths still appears in the Common content footer."
Assert-NotContains $Source 'footer-actions[^\r\n]*applyCurrentProgram' "Current-tab Apply still appears in the content footer."
Assert-Contains $Source '\.field\.valid\s*\{\s*color: #76df96;\s*\}' "Valid Config paths do not keep green text with the neutral field border."
Assert-NotContains $Source '\.field\.valid\s*\{[^}]*border-color' "Valid Config paths still use a green border."
Assert-Contains $Source '\.field\.invalid\s*\{\s*border-color: #80434b;' "Missing Config paths no longer use the red border."
Assert-Contains $Source 'function moveDynamicLink\(tabIndex, index, direction\)' "Links row move handler from v0.71 is missing."
Assert-Contains $Source 'oncontextmenu="return moveDynamicLinkUp\(' "Links row right-click move-up action from v0.71 is missing."
Assert-Contains $Source '\.button-apply-action,\s*\.button-apply-action:hover\s*\{\s*border-color: #34794c;\s*color: #76df96;' "Apply action buttons do not retain the v0.68 green accent."
Assert-Contains $Source '[.]button-apply-action[.]button-apply-running,' "Running Apply button selector is missing."
Assert-Contains $Source 'background: #2a2d32;' "Running Apply buttons do not retain the dark disabled surface."
Assert-Contains $Source 'color: #686d75;' "Running Apply buttons are not greyed out."
Assert-Contains $Source 'var applyInProgress = false;' "Shared Apply running-state guard is missing."
Assert-Contains $Source 'function updateApplyActionButtons[(][)]' "Apply button running-state synchronizer is missing."
Assert-Contains $Source 'button[.]disabled = applyInProgress;' "Apply buttons are not genuinely disabled during Apply."
Assert-Contains $Source 'function runApplyAction[(]action[)]' "Apply action wrapper is missing."
Assert-Contains $Source 'window[.]setTimeout[(]function[(][)]' "Apply work is not deferred to allow the disabled appearance to repaint."
Assert-Contains $Source 'finally [{]' "Apply running state is not restored through a guaranteed cleanup path."
Assert-Contains $Source 'updateApplyActionButtons[(][)];' "Rerendered Apply buttons do not retain the running state."
Assert-Contains $Source 'onclick="return runApplyAction[(]applyAll[)]">APPLY ALL' "Apply All does not use the shared running-state wrapper."
if ([regex]::Matches($Source, 'class="button button-apply-action"').Count -ne 3) {
    $Failures.Add("Apply All, Apply Paths, and Apply Current Program do not all use the green action class.")
}
Assert-Contains $Source 'function applyPaths\(\)[\s\S]*?if \(result\.error_count === 0\) \{\s*render\(true\);\s*\} else \{\s*render\(\);\s*\}' "Apply Paths does not refresh successful live state while preserving its existing error redraw."
Assert-Contains $Source 'function applyAll\(\)[\s\S]*?if \(persistentResult\.error_count === 0 && dynamicErrors === 0\) \{\s*render\(true\);\s*\} else \{\s*render\(\);\s*\}' "Apply All does not refresh successful live state while preserving its existing error redraw."
Assert-Contains $Source 'function applyCurrentProgram\(\)[\s\S]*?if \(result\.errors === 0\) \{\s*render\(true\);\s*\}' "Apply Current Program does not refresh the UI after success."
Assert-Contains $Source 'errorsLogPath = fso[.]BuildPath[(]appDir, "ERRORS-Log[.]md"[)];' "ERRORS-Log.md is not created beside Path-Config."
Assert-Contains $Source 'function resetLog[(][)][\s\S]*?applyErrorLogCount = 0;[\s\S]*?fso[.]DeleteFile[(]errorsLogPath, true[)];' "A new Apply run does not clear stale error-log state."
Assert-Contains $Source 'function appendErrorLog[(]message[)][\s\S]*?writeText[(]errorsLogPath,[\s\S]*?"- FAILED: " [+] message' "Failed Apply entries are not written to ERRORS-Log.md."
Assert-Contains $Source 'function appendLog[(]message, failed[)][\s\S]*?if [(]failed[)] [{]\s*appendErrorLog[(]message[)];' "Ordinary Apply error logging does not feed ERRORS-Log.md."
Assert-Contains $Source 'function openErrorsLogIfNeeded[(][)][\s\S]*?applyErrorLogCount === 0[\s\S]*?wsh[.]Run[(]quoteArg[(]errorsLogPath[)], 1, false[)]' "ERRORS-Log.md is not conditionally opened after errors."
if ([regex]::Matches($Source, 'openErrorsLogIfNeeded[(][)];').Count -ne 3) {
    $Failures.Add("Apply Paths, Apply All, and Apply Current Program do not all open the error log after reporting failures.")
}
Assert-Contains $Source 'return title [+] "\\n\\nCompleted:[\s\S]*?"\\n\\nEnvironment updates:[\s\S]*?"\\n\\nErrors:' "Apply Paths summary lines are not divided by blank lines."
Assert-Contains $Source 'alert[(]"Apply All\\n\\nPersistent actions completed:[\s\S]*?"\\n\\nPersistent errors:[\s\S]*?"\\n\\nDynamic errors:' "Apply All summary lines are not divided by blank lines."
Assert-Contains $Source 'alert[(]"Apply " [+] name [+] "\\n\\nStart11 menu pins ready:[\s\S]*?"\\n\\nStart11 shortcuts removed:[\s\S]*?"\\n\\nCompleted with ' "Current-program summary lines are not divided by blank lines."
...
Assert-Contains $Source 'sameDirectory = linkDirectoriesAreSame\(row\.src, row\.tgt_dir\);[\s\S]*?readonlyCell\(row\.src, sameDirectory \? "same-directory" : "sourcepath"\)[\s\S]*?readonlyCell\(target, sameDirectory \? "same-directory" : "targetpath"\)' "Links APPLY rows do not preserve same-folder warnings and distinct source/target validation colors."
...
Assert-Contains $Source 'function linkBackupPath[(]path[)][\s\S]*?return path [+] "[.]default";' "Canonical .default Link backup naming is missing."
Assert-Contains $Source 'function removeTargetReparsePoint\(path\)' "Destination reparse-point unlinking is missing."
Assert-Contains $Source '\$item\.Attributes -band \[IO\.FileAttributes\]::ReparsePoint' "Link cleanup does not require an actual reparse point."
Assert-Contains $Source '\[IO\.Directory\]::Delete\(\$path,\$false\)' "Directory links are not unlinked without recursive deletion."
Assert-Contains $Source '\[IO\.File\]::Delete\(\$path\)' "File links are not unlinked without touching their sources."
Assert-Contains $Source 'function assertSafeLinkPaths\(sourcePath, targetPath, sourceIsDirectory\)' "Source/destination overlap guard is missing."
Assert-Contains $Source 'assertSafeLinkPaths\(src, targetPath, isDirectory\);[\s\S]*?configuredReparsePointStatus\(targetPath, src\)' "Symlink status is inspected before the source-safety guard."
Assert-Contains $Source 'configuredReparsePointStatus\(targetPath, src\)[\s\S]*?Symlink already correct' "Correct existing symlinks are not preserved."
Assert-Contains $Source 'preparedPath = temporaryLinkPath\(targetPath\);[\s\S]*?runMklink\(command\);[\s\S]*?prepareLinkTarget\(targetPath\);[\s\S]*?movePreparedLink\(preparedPath, targetPath, isDirectory\);' "Link replacement is not prepared before destination mutation."
Assert-Contains $Source 'assertSafeLinkPaths\(src, shortcutPath, fso[.]FolderExists\(src\)\);[\s\S]*?installPreparedShortcut\(preparedShortcut, shortcutPath\);' "Shortcut installation is not source-guarded and transactional."
Assert-NotContains $Source 'fso[.]DeleteFile\(shortcutPath' "Shortcut creation still deletes its destination before a replacement is prepared."
Assert-Contains $Source 'writeText\(helperPath, "@echo off\\r\\n" \+ mklinkArguments' "Elevated mklink retry does not preserve its exact command in a helper file."
Assert-Contains $Source 'powerShellLiteral\(quoteArg\(helperPath\)\)' "Elevated mklink helper path is not passed as one quoted argument."
Assert-Contains $Source 'function installPreparedShortcut\(preparedPath, targetPath\)' "Transactional Shortcut installer is missing."
Assert-NotContains $Source 'Remove-Item -LiteralPath' "Link cleanup still uses recursive-capable Remove-Item."
Assert-Contains $Source 'function runMklink\(mklinkArguments\)' "Shared mklink runner is missing."
Assert-Contains $Source 'Start-Process -FilePath \$env:ComSpec[\s\S]*?-Verb RunAs[\s\S]*?-Wait -PassThru' "Failed symbolic-link creation is not retried with UAC elevation."
Assert-Contains $Source 'function linkBackupExists[(]path[)][\s\S]*?fso[.]FileExists[(]backupPath[)] [|][|] fso[.]FolderExists[(]backupPath[)]' "Canonical Link backup detection does not check both files and folders."
Assert-Contains $Source 'else if [(]backupExists[)] [{]\s*fso[.]DeleteFile[(]path, true[)];' "A regular file target is not removed when its canonical .default backup already exists."
Assert-NotContains $Source 'fso[.]DeleteFolder[(]' "Link apply still contains recursive directory deletion."
Assert-Contains $Source 'Refusing to recursively delete an existing Link destination directory' "Occupied destination directories do not fail safely."
Assert-Contains $Source 'fso[.]MoveFile[(]path, linkBackupPath[(]path[)][)];' "A regular file target is not preserved as the canonical .default backup."
Assert-Contains $Source 'fso[.]MoveFolder[(]path, linkBackupPath[(]path[)][)];' "A regular folder target is not preserved as the canonical .default backup."
Assert-NotContains $Source 'candidate = base;|availableLinkBackupPath|candidate = base [+] "[.]" [+] suffix' "Numbered or collision-generated Link backups can still be created."
...
Assert-Contains $Source 'state\.mode === "Config" && normalizePaths[\s\S]*?normalizeConfigPathValue\(element, value\)' "Path normalization is not limited to Config-mode focus-loss capture."
...
Assert-Contains $Source 'function normalizePathSeparators\(value\)' "Central path-separator normalizer is missing."
Assert-Contains $Source 'return normalizePathSeparators\(expandEnvironmentText' "Expanded paths are not normalized after variable substitution."
Assert-Contains $Source 'value = normalizePathSeparators\(value\)' "Config-mode paths do not use central separator normalization."
...
Assert-Contains $Source 'captureElement\(target, eventObject\.type === "focusout"\)' "Live edits are not deferred from normalization until focus loss."
Assert-Contains $Source 'document\.attachEvent\("onfocusout", trackInputChange\)' "HTA path controls do not normalize on focus loss."
...
Assert-Contains $Source 'function canonicalStart11ShortcutPath\(sourceInfo\)' "Canonical Start11 shortcut-path resolver is missing."
Assert-Contains $Source 'return fso\.BuildPath\(directory, baseName \+ "\.lnk"\);' "Start11 addition does not select the unsuffixed canonical shortcut name."
Assert-Contains $Source 'function cleanupStart11ShortcutConflicts\(sourceInfo, shortcutPath, result, rowLabel\)' "Start11 shortcut conflict cleanup is missing."
Assert-Contains $Source 'fileKey === canonicalKey && fileSignature !== sourceInfo\.signature' "Same-name Start11 shortcut conflicts are not selected for cleanup."
Assert-Contains $Source 'fileKey !== canonicalKey && fileSignature === sourceInfo\.signature' "Duplicate requested-target Start11 shortcuts are not selected for cleanup."
Assert-Contains $Source 'fso\.DeleteFile\(deletePaths\[i\], false\)' "Conflicting Start11 shortcut files are not deleted."
Assert-NotContains $Source 'function availableStart11ShortcutPath' "Legacy numbered Start11 shortcut collision selection remains."
...
Assert-Contains $Source 'if \(captureElement\(target, eventObject\.type === "focusout"\)\) \{\s*markDirty\(\);' "Live dialog edits do not mark only real value changes dirty."
...
if ($Source.IndexOf('link_name: iniValue(ini, "PersistentPaths", i + "_LinkName", "")') -lt 0) { $Failures.Add("Common Start Menu Name loading is missing.") }
if ($Source.IndexOf('entry(lines, (i + 1) + "_LinkName", row.link_name || "")') -lt 0) { $Failures.Add("Common Start Menu Name saving is missing.") }
if ($Source.IndexOf('fieldInput("persistent", "", i, "link_name", row.link_name || "", "")') -lt 0) { $Failures.Add("Common Start Menu Name edit is missing.") }
$CommonHeaderOrder = '<thead><tr><th></th><th></th><th>File path</th><th>Environment variable</th>'
if ($Source.IndexOf($CommonHeaderOrder) -lt 0) { $Failures.Add("Common headers do not place ADMIN, STARTUP, MENU, and START MENU NAME after Environment variable.") }
$CommonRenderStart = $Source.IndexOf('function renderPersistent()')
$CommonRenderEnd = $Source.IndexOf('function renderProgram(', $CommonRenderStart)
$CommonRender = $Source.Substring($CommonRenderStart, $CommonRenderEnd - $CommonRenderStart)
$ConfigEnvIndex = $CommonRender.IndexOf('fieldInput("persistent", "", i, "env_var"')
$ConfigAdminIndex = $CommonRender.IndexOf('checkboxInput("persistent", "", i, "run_as_admin"')
$ConfigStartupIndex = $CommonRender.IndexOf('checkboxInput("persistent", "", i, "run_on_startup"')
$ConfigMenuIndex = $CommonRender.IndexOf('checkboxInput("persistent", "", i, "menu"')
$ConfigNameIndex = $CommonRender.IndexOf('fieldInput("persistent", "", i, "link_name"')
if ($ConfigEnvIndex -lt 0 -or $ConfigEnvIndex -gt $ConfigAdminIndex -or $ConfigAdminIndex -gt $ConfigStartupIndex -or $ConfigStartupIndex -gt $ConfigMenuIndex -or $ConfigMenuIndex -gt $ConfigNameIndex) { $Failures.Add("Common Config controls are not ordered after Environment variable.") }
$ApplyEnvIndex = $CommonRender.IndexOf('readonlyCell(row.env_var')
$ApplyAdminIndex = $CommonRender.IndexOf('adminPropertyMatchCell(row.path')
$ApplyStartupIndex = $CommonRender.IndexOf('startupMatchCell(row.path')
$ApplyMenuIndex = $CommonRender.IndexOf('menuPinMatchCell(row.path')
$ApplyNameIndex = $CommonRender.IndexOf('readonlyCell(row.link_name')
if ($ApplyEnvIndex -lt 0 -or $ApplyEnvIndex -gt $ApplyAdminIndex -or $ApplyAdminIndex -gt $ApplyStartupIndex -or $ApplyStartupIndex -gt $ApplyMenuIndex -or $ApplyMenuIndex -gt $ApplyNameIndex) { $Failures.Add("Common Apply controls are not ordered after Environment variable.") }
if ($Source.IndexOf('out += readonlyCell(row.link_name || "", "");') -lt 0) { $Failures.Add("Common Apply mode does not show Start Menu Name.") }
if ($Source.IndexOf('ensureStart11MenuPin(path, row.run_as_admin ? true : false, result, "Row " + (i + 1), row.link_name || "")') -lt 0) { $Failures.Add("Common Apply does not use its Start Menu Name.") }
if ($Source.IndexOf('link_name: row.link_name || ""') -lt 0) { $Failures.Add("Move to New Tab does not transfer Common Start Menu Name.") }
if ([regex]::Matches($Source, '<th>START MENU NAME</th>').Count -lt 2) {
    $Failures.Add("START MENU NAME is not shown in both Common and program Paths.")
}
...
if (-not $ScriptMatch.Success) {
    ...
    try {
        $MigrationCheck = @'
var originalErrorLogTestFso = fso;
var originalErrorLogTestWsh = wsh;
var errorLogTestFiles = {};
var openedErrorLogPath = "";
function errorLogTestStream(path) {
    return {
        Write: function(value) { errorLogTestFiles[path] = (errorLogTestFiles[path] || "") + value; },
        Close: function() {}
    };
}
fso = {
    FileExists: function(path) { return Object.prototype.hasOwnProperty.call(errorLogTestFiles, path); },
    DeleteFile: function(path, force) { delete errorLogTestFiles[path]; },
    CreateTextFile: function(path, overwrite, unicode) {
        errorLogTestFiles[path] = "";
        return errorLogTestStream(path);
    },
    OpenTextFile: function(path, mode, create, format) {
        if (!Object.prototype.hasOwnProperty.call(errorLogTestFiles, path)) {
            errorLogTestFiles[path] = "";
        }
        return errorLogTestStream(path);
    }
...
wsh = {
    Run: function(command, style, wait) {
        openedErrorLogPath = command;
        return 0;
    }
};
logPath = "C:\\Test\\Path-Config-Apply-Log.md";
errorsLogPath = "C:\\Test\\ERRORS-Log.md";
resetLog();
appendLog("Successful action", false);
openErrorsLogIfNeeded();
if (fso.FileExists(errorsLogPath) || openedErrorLogPath) {
    throw new Error("A successful Apply entry created or opened ERRORS-Log.md.");
}
appendLog("Failed action", true);
if (!fso.FileExists(errorsLogPath) ||
        errorLogTestFiles[errorsLogPath].indexOf("- FAILED: Failed action") < 0 ||
        errorLogTestFiles[errorsLogPath].indexOf("Successful action") >= 0) {
    throw new Error("ERRORS-Log.md did not contain only the failed Apply entry.");
}
openErrorsLogIfNeeded();
if (openedErrorLogPath !== quoteArg(errorsLogPath)) {
    throw new Error("ERRORS-Log.md was not opened after an Apply failure.");
}
resetLog();
if (fso.FileExists(errorsLogPath) || applyErrorLogCount !== 0) {
    throw new Error("A new Apply run did not remove the stale ERRORS-Log.md.");
}
fso = originalErrorLogTestFso;
wsh = originalErrorLogTestWsh;
var originalLinkTestFso = fso;
var originalLinkTestWsh = wsh;
var linkTestCommands = [];
wsh = {
    Run: function(command, style, wait) {
        linkTestCommands.push({command: command, style: style, wait: wait});
        return 0;
    }
};
fso = {
    FileExists: function() { throw new Error("Reparse cleanup did not return before FileExists."); },
    FolderExists: function() { throw new Error("Reparse cleanup did not return before FolderExists."); }
};
prepareLinkTarget("C:\\BrokenLink");
if (linkTestCommands.length !== 1 || linkTestCommands[0].command.indexOf("Get-Item -LiteralPath") < 0 ||
        linkTestCommands[0].command.indexOf("[IO.Directory]::Delete($path,$false)") < 0 ||
        linkTestCommands[0].command.indexOf("[IO.File]::Delete($path)") < 0 ||
        linkTestCommands[0].command.indexOf("Remove-Item") >= 0 ||
        linkTestCommands[0].style !== 0 || linkTestCommands[0].wait !== true) {
    throw new Error("Destination reparse point was not unlinked non-recursively and hidden.");
}
var guardedLinkOverlaps = 0;
try {
    assertSafeLinkPaths("D:\\Source\\", "D:\\Source\\NestedLink", true);
} catch (expectedNestedTargetGuard) {
    guardedLinkOverlaps++;
}
try {
    assertSafeLinkPaths("D:\\Source\\Nested", "D:\\Source", true);
} catch (expectedAncestorTargetGuard) {
    guardedLinkOverlaps++;
}
try {
    assertSafeLinkPaths("D:\\Source\\", "D:\\Source\\", true);
} catch (expectedSameTargetGuard) {
    guardedLinkOverlaps++;
}
assertSafeLinkPaths("D:\\Source\\", "C:\\Target\\Source", true);
if (guardedLinkOverlaps !== 3 || normalizedLinkSafetyPath("D:\\Source\\\\") !== "d:\\source") {
    throw new Error("Source/destination overlap guard failed for trailing separators or nested paths.");
}
wsh = { Run: function() { return 2; } };
var unsafeInspectionStopped = false;
try { prepareLinkTarget("C:\\UninspectableLink"); }
catch (expectedInspectionFailure) { unsafeInspectionStopped = expectedInspectionFailure.message.indexOf("Could not safely inspect") >= 0; }
if (!unsafeInspectionStopped) {
    throw new Error("A destination inspection failure did not stop before filesystem fallback handling.");
}
linkTestCommands = [];
var linkHelperContent = "";
var deletedLinkHelper = "";
fso = {
    BuildPath: function(dir, name) { return dir + "\\" + name; },
    FileExists: function(path) { return path.indexOf("PathConfig-Mklink-") >= 0 && !deletedLinkHelper; },
    CreateTextFile: function(path) {
        return {
            Write: function(value) { linkHelperContent = value; },
            Close: function() {}
        };
    },
    DeleteFile: function(path) { deletedLinkHelper = path; }
};
wsh = {
    ExpandEnvironmentStrings: function() { return "C:\\Temp"; },
    Run: function(command, style, wait) {
        linkTestCommands.push({command: command, style: style, wait: wait});
        return linkTestCommands.length === 1 ? 1 : 0;
    }
};
if (runMklink('mklink "C:\\Target" "D:\\Source"') !== 0 || linkTestCommands.length !== 2 ||
        linkTestCommands[1].command.indexOf("Start-Process") < 0 ||
        linkTestCommands[1].command.indexOf("PathConfig-Mklink-") < 0 ||
        linkTestCommands[1].command.indexOf("-Verb RunAs") < 0 ||
        linkTestCommands[1].style !== 0 || linkTestCommands[1].wait !== true) {
    throw new Error("mklink privilege failure was not retried through a quoted hidden UAC helper.");
}
if (linkHelperContent.indexOf('mklink "C:\\Target" "D:\\Source"') < 0 || !deletedLinkHelper) {
    throw new Error("Elevated mklink helper did not preserve its exact command or clean itself up.");
}
wsh = { Run: function() { return 1; } };
var movedLinkSource = "";
var movedLinkBackup = "";
var deletedLinkTarget = "";
fso = {
    FileExists: function(path) {
        return path === "C:\\Target\\settings.ini" ||
            path === "C:\\Target\\settings.ini.default";
    },
    FolderExists: function(path) { return false; },
    GetFile: function(path) { return {Attributes: 0}; },
    MoveFile: function(source, target) {
        movedLinkSource = source;
        movedLinkBackup = target;
    },
    DeleteFile: function(path, force) {
        deletedLinkTarget = path;
    }
};
prepareLinkTarget("C:\\Target\\settings.ini");
wsh = originalLinkTestWsh;
if (movedLinkSource || movedLinkBackup ||
        deletedLinkTarget !== "C:\\Target\\settings.ini") {
    throw new Error("Existing .default backup was not preserved as the only Link backup.");
}
fso = {
...
var menuDuplicatePath = "C:\\Pinned\\Tool (2).lnk";
var menuConflictTargetPath = "C:\\Other\\Different.exe";
var menuFiles = {};
menuFiles[menuSourcePath] = true;
menuFiles[menuTargetPath] = true;
menuFiles[menuPinnedPath] = true;
menuFiles[menuDuplicatePath] = true;
var menuShortcutDetails = {};
menuShortcutDetails[menuSourcePath] = {target: menuTargetPath, arguments: "--profile Work"};
menuShortcutDetails[menuPinnedPath] = {target: menuConflictTargetPath, arguments: ""};
menuShortcutDetails[menuDuplicatePath] = {target: menuTargetPath, arguments: "--profile Work"};
var menuCopiedFrom = "";
var menuCopiedTo = "";
var menuDeletedPaths = [];
var menuGroupValues = [[], []];
var menuCustomValues = [[], []];
var savedMenuEnumerator = Enumerator;
...
function menuFolderFiles() {
    var items = [];
    var path;
    for (path in menuFiles) {
        if (menuFiles[path] && fso.GetParentFolderName(path) === "C:\\Pinned" &&
                lower(fso.GetExtensionName(path)) === "lnk") {
...
fso = {
    ...
    GetFolder: function(directory) { return {Files: menuFolderFiles()}; },
    DeleteFile: function(path, force) {
        if (!menuFiles[path]) { throw new Error("Missing shortcut"); }
        menuDeletedPaths.push(path);
        delete menuFiles[path];
        delete menuShortcutDetails[path];
    },
    CopyFile: function(sourcePath, destinationPath, overwrite) {
        if (overwrite || !menuFiles[destinationPath]) {
            ...
            menuShortcutDetails[destinationPath] = {
                target: menuShortcutDetails[sourcePath].target,
                arguments: menuShortcutDetails[sourcePath].arguments
            };
            return;
        }
...
wsh = {
    ...
    CreateShortcut: function(path) {
        if (!menuFiles[path] || lower(fso.GetExtensionName(path)) !== "lnk" || !menuShortcutDetails[path]) {
            throw new Error("Missing shortcut");
        }
        return {
            TargetPath: menuShortcutDetails[path].target,
            Arguments: menuShortcutDetails[path].arguments
        };
    },
    ...
    RegDelete: function(registryPath) {
        if (registryPath.indexOf("\\CustomGroup\\") >= 0) {
            var customGroupIndex = registryPath.indexOf(START11_PIN_ROOTS[1]) === 0 ? 1 : 0;
            menuCustomValues[customGroupIndex] = [];
            return;
        }
        var groupIndex = registryPath.indexOf(START11_PIN_GROUPS[1].key) === 0 ? 1 : 0;
...
readStart11PinTreeValues = function(registryRoot) {
    var groupIndex = registryRoot === START11_PIN_ROOTS[1] ? 1 : 0;
    return menuCustomValues[groupIndex].concat(menuGroupValues[groupIndex]);
};
appendLog = function() {};
...
menuGroupValues[0].push({
    registry_key: START11_PIN_GROUPS[0].key,
    name: "0",
    data: menuPinnedPath + START11_PIN_GROUPS[0].suffix,
    shortcut_path: menuPinnedPath
});
menuGroupValues[1].push({
    registry_key: START11_PIN_GROUPS[1].key,
    name: "0",
    data: menuDuplicatePath + START11_PIN_GROUPS[1].suffix,
    shortcut_path: menuDuplicatePath
});
...
if (menuCopiedFrom !== menuSourcePath || menuCopiedTo !== menuPinnedPath ||
        menuDeletedPaths.length !== 2 || menuDeletedPaths[0] !== menuPinnedPath ||
        menuDeletedPaths[1] !== menuDuplicatePath || menuFiles[menuDuplicatePath] ||
        menuGroupValues[0].length !== 1 || menuGroupValues[1].length !== 1 ||
        menuGroupValues[0][0].shortcut_path !== menuPinnedPath ||
        menuGroupValues[1][0].shortcut_path !== menuPinnedPath ||
        menuResult.menu_pin_count !== 1 || menuResult.menu_created_count !== 1 ||
        menuResult.menu_removed_count !== 1 || menuResult.success_count !== 1 ||
        menuResult.error_count !== 0 || !isStart11MenuSourcePinned(menuSourcePath)) {
    throw new Error("Conflicting and duplicate Start11 shortcuts were not replaced by one canonical shortcut.");
}
menuGroupValues[0][0].registry_key = START11_PIN_ROOTS[0] + "\\CustomGroup";
menuCustomValues[0].push(menuGroupValues[0][0]);
menuGroupValues[0] = [];
start11PinCache = null;
ensureStart11MenuPin(menuSourcePath, false, menuResult, "Shortcut row");
if (menuGroupValues[0].length !== 0 || menuCustomValues[0].length !== 1 || menuGroupValues[1].length !== 1 ||
...
menuGroupValues[1][0].registry_key = START11_PIN_ROOTS[1] + "\\CustomGroup";
menuCustomValues[1].push(menuGroupValues[1][0]);
menuGroupValues[1] = [];
start11PinCache = null;
ensureStart11MenuPin(menuSourcePath, false, menuResult, "Shortcut row");
if (menuGroupValues[0].length !== 0 || menuGroupValues[1].length !== 0 ||
        menuCustomValues[0].length !== 1 || menuCustomValues[1].length !== 1 ||
        menuResult.menu_pin_count !== 3 || menuResult.error_count !== 0) {
    throw new Error("Repeated Start11 add recreated a default-group duplicate after both pins moved into custom groups.");
}
...
removeStart11MenuPin(menuSourcePath, menuResult, "Shortcut row");
if (menuGroupValues[0].length !== 0 || menuCustomValues[0].length !== 0 ||
        menuGroupValues[1].length !== 0 || menuCustomValues[1].length !== 0 ||
        menuResult.menu_removed_count !== 3 || isStart11MenuSourcePinned(menuSourcePath)) {
...
start11PinCache = null;
Enumerator = savedMenuEnumerator;
fso = savedMenuFso;
wsh = savedMenuWsh;
...
captureElement(configPathElement, false);
if (configPathElement.value !== "c:\\Users\\vilbur\\\\.gk" || state.persistent[0].path !== "c:\\Users\\vilbur\\\\.gk") {
    throw new Error("Config-mode capture sanitized a dotted path while it was still being edited.");
}
captureElement(configPathElement, true);
if (configPathElement.value !== "C:\\Users\\vilbur\\.gk" || state.persistent[0].path !== "C:\\Users\\vilbur\\.gk") {
    throw new Error("Config-mode focus loss did not collapse a doubled separator before a dotted directory.");
}
...
if (sanitizeBrowsedPath("d:\\Tools\\Tool.exe", false) !== "D:\\Tools\\Tool.exe" ||
        ...
        sanitizeBrowsedPath("e:\\Data", true) !== "E:\\Data\\" ||
        sanitizeBrowsedPath("D:\\\\Tools\\\\.Hidden", false) !== "D:\\Tools\\.Hidden") {
    throw new Error("Browsed-path sanitization did not normalize drive letters and folder endings safely.");
}
if (expandPathVariables("%GoogleDrive%\\ProgramsData", {"googledrive": "D:\\GoogleDrive\\"}) !== "D:\\GoogleDrive\\ProgramsData") {
    throw new Error("Separator duplication across an environment-variable boundary was not removed.");
}
...
function makeApplyPersistentResult(errorCount) {
    return {
        env_changed: false,
        admin_property_count: 0,
        admin_removed_count: 0,
        startup_count: 0,
        menu_pin_count: 0,
        menu_created_count: 0,
        menu_removed_count: 0,
...
function makeApplyTabResult(errorCount) {
    return {
        env_changed: false,
        errors: errorCount,
        error_count: 0,
...
var applyRenderArguments = [];
var currentPersistentApplyResult = makeApplyPersistentResult(0);
var currentTabApplyResult = makeApplyTabResult(0);
captureState = function() {};
resetLog = function() {};
broadcastEnvironmentChange = function() {};
cleanupProgramStartup = function(prefix, result) {};
applyPersistentPaths = function() { return currentPersistentApplyResult; };
applyTabConfig = function() { return currentTabApplyResult; };
render = function(preserveWindowSize) { applyRenderArguments.push(preserveWindowSize); };
alert = function() {};
state.tabs = ["PROGRAM"];
state.activeUiTab = 1;
state.activeProgramTab = 0;
applyPaths();
currentPersistentApplyResult = makeApplyPersistentResult(1);
applyPaths();
currentPersistentApplyResult = makeApplyPersistentResult(0);
currentTabApplyResult = makeApplyTabResult(0);
applyAll();
currentTabApplyResult = makeApplyTabResult(1);
applyAll();
currentTabApplyResult = makeApplyTabResult(0);
applyCurrentProgram();
var renderCountBeforeFailedCurrentApply = applyRenderArguments.length;
currentTabApplyResult = makeApplyTabResult(1);
applyCurrentProgram();
if (applyRenderArguments.length !== 5 || applyRenderArguments[0] !== true ||
        typeof applyRenderArguments[1] !== "undefined" || applyRenderArguments[2] !== true ||
        typeof applyRenderArguments[3] !== "undefined" || applyRenderArguments[4] !== true ||
        applyRenderArguments.length !== renderCountBeforeFailedCurrentApply) {
    throw new Error("Apply actions did not refresh successful UI state with the required error behavior.");
}
...
var runningApplyButton = {className: "button button-apply-action", disabled: false};
var ordinaryButton = {className: "button", disabled: false};
var scheduledApply = null;
var scheduledApplyDelay = -1;
var applyCalls = 0;
document = {
    getElementsByTagName: function(name) {
        return name === "button" ? [runningApplyButton, ordinaryButton] : [];
    }
};
window = {
    setTimeout: function(callback, delay) {
        scheduledApply = callback;
        scheduledApplyDelay = delay;
    }
};
applyInProgress = false;
var firstApplyResult = runApplyAction(function() {
    applyCalls++;
    if (!runningApplyButton.disabled ||
            runningApplyButton.className.indexOf("button-apply-running") < 0) {
        throw new Error("Apply button was restored before Apply work completed.");
    }
});
var duplicateApplyResult = runApplyAction(function() { applyCalls += 100; });
if (firstApplyResult !== false || duplicateApplyResult !== false ||
        applyCalls !== 0 || typeof scheduledApply !== "function" || scheduledApplyDelay !== 0 ||
        !applyInProgress || !runningApplyButton.disabled ||
        runningApplyButton.className.indexOf("button-apply-running") < 0 ||
        ordinaryButton.disabled) {
    throw new Error("Apply did not immediately disable and grey only the Apply buttons before deferred work.");
}
scheduledApply();
if (applyCalls !== 1 || applyInProgress || runningApplyButton.disabled ||
        runningApplyButton.className.indexOf("button-apply-running") >= 0) {
    throw new Error("Apply buttons were not restored after successful Apply work.");
}
var failedApplyCaught = false;
runApplyAction(function() { throw new Error("Expected Apply failure"); });
try {
    scheduledApply();
} catch (expectedApplyFailure) {
    failedApplyCaught = true;
}
if (!failedApplyCaught || applyInProgress || runningApplyButton.disabled ||
        runningApplyButton.className.indexOf("button-apply-running") >= 0) {
    throw new Error("Apply buttons were not restored after failed Apply work.");
}
'@
