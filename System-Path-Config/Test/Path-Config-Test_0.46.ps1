...
Assert-Contains $Source 'var VERSION = "0\.46";' "HTA internal version is not 0.46."
...
Assert-Contains $Source '\.button\[disabled\]\s*\{\s*color: #686d75;\s*cursor: default;\s*\}' "Disabled Save State does not use grey text on the original button surface."
Assert-NotContains $Source '\.button\[disabled\],' "Disabled Save State still replaces the original border or background styling."
Assert-NotContains $Source '\.button\[disabled\]:hover' "Disabled Save State still has a custom hover surface."
Assert-Contains $Source 'id="saveStateButton"[\s\S]*?hasUnsavedChanges \? '''' : '' disabled="disabled"''' "Save State button is not disabled when nothing changed."
Assert-Contains $Source 'function updateSaveStateButton\(\)' "Live Save State button synchronization is missing."
Assert-Contains $Source 'hasUnsavedChanges = false;\s*updateSaveStateButton\(\);' "Successful saving does not disable Save State immediately."
...
Assert-Contains $Source 'readonlyCell\(row\.src, "pathcheck"\) \+ readonlyCell\(target, ""\) \+ readonlyCell\(row\.type, ""\)' "Links APPLY rows do not contain exactly Source, combined Link path, and Type values."
...
Assert-Contains $Source 'rows\.push\(\{src: "", tgt_dir: previousLinkFolder, tgt_name: "", type: "Shortcut"\}\);' "New Links rows do not preserve clean defaults while inheriting Link folder."
...
if (!linkPrefillDirty || !linkPrefillRendered || inheritedLink.src !== "" ||
        ...
        inheritedLink.type !== "Shortcut") {
    throw new Error("New Link row did not inherit only the previous non-empty Link folder.");
}
...
var saveButtonState = {disabled: null};
document = {
    getElementById: function(id) {
...
if (saveButtonState.disabled !== true) {
    throw new Error("Save State button was not disabled for a clean state.");
}
hasUnsavedChanges = true;
updateSaveStateButton();
if (saveButtonState.disabled !== false) {
    throw new Error("Save State button was not enabled for an unsaved state.");
}
var modeSaveCalls = 0;
var modeRendered = 0;
var modeStatus = "";
