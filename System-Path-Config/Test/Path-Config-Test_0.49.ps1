...
Assert-Contains $Source 'var VERSION = "0\.49";' "HTA internal version is not 0.49."
...
Assert-Contains $Source '\.button\s*\{\s*min-width: 126px;[\s\S]*?border: 1px solid #444850;\s*background: #2a2d32;\s*color: #f0f1f2;\s*cursor: pointer;\s*\}' "The v0.39 base button surface was not preserved."
Assert-Contains $Source '\.save-state-frozen\s*\{\s*color: #686d75;\s*cursor: default;\s*\}' "Frozen Save State does not use grey text on the v0.39 surface."
...
Assert-Contains $Source 'id="saveStateButton"[\s\S]*?save-state-frozen[\s\S]*?saveStateIfAvailable\(\)' "Save State does not render with the conditional frozen class and guarded handler."
...
Assert-Contains $Source 'function saveStateIfAvailable\(\)[\s\S]*?if \(hasUnsavedChanges\)[\s\S]*?saveState\(true\)' "Frozen Save State clicks are not guarded."
...
if (saveButtonState.className !== "button save-state-frozen") {
    throw new Error("Save State button was not frozen with the grey-text class for a clean state.");
}
hasUnsavedChanges = true;
updateSaveStateButton();
if (saveButtonState.className !== "button") {
    throw new Error("Save State button did not restore the exact base class for an unsaved state.");
}
var guardedSaveCalls = 0;
saveState = function(showMessage) {
    if (showMessage !== true) {
        throw new Error("Manual guarded save did not request its confirmation message.");
    }
    guardedSaveCalls++;
    return true;
};
...
if (guardedSaveCalls !== 0) {
    throw new Error("Frozen Save State accepted a click with no pending changes.");
}
hasUnsavedChanges = true;
saveStateIfAvailable();
if (guardedSaveCalls !== 1) {
    throw new Error("Available Save State did not execute its manual save.");
}
var modeSaveCalls = 0;
var modeRendered = 0;
var modeStatus = "";
