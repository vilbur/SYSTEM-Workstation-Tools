...
Assert-Contains $Source 'var VERSION = "0\.52";' "HTA internal version is not 0.52."
...
if ([regex]::Matches($Source, '<th>Menu</th>').Count -lt 2) {
    $Failures.Add("Menu header is not used beside Startup in both path tables.")
}
Assert-Contains $Source '<th>Admin</th><th>Startup</th><th>Menu</th>' "Menu is not positioned directly after Startup."
...
if ([regex]::Matches($Source, '<col style="width:40px"><col style="width:40px"><col style="width:40px"><col style="width:40px">').Count -lt 2) {
    $Failures.Add("Path tables do not use compact Admin, Startup, Menu, and right-edge columns.")
}
