...
Assert-Contains $Source 'var VERSION = "0\.77";' "HTA internal version is not 0.77."
...
Assert-Contains $Source 'function availableLinkBackupPath[(]path[)]' "Collision-safe Link backup naming is missing."
Assert-Contains $Source 'while [(]fso[.]FileExists[(]candidate[)] [|][|] fso[.]FolderExists[(]candidate[)][)]' "Link backup naming does not check both file and folder collisions."
Assert-Contains $Source 'candidate = base [+] "[.]" [+] suffix;' "Link backup naming does not add a numeric suffix."
Assert-Contains $Source 'suffix[+][+];' "Link backup naming does not advance through occupied suffixes."
Assert-Contains $Source 'fso[.]MoveFile[(]path, availableLinkBackupPath[(]path[)][)];' "Regular file Link targets are not preserved under an available backup name."
Assert-Contains $Source 'fso[.]MoveFolder[(]path, availableLinkBackupPath[(]path[)][)];' "Regular folder Link targets are not preserved under an available backup name."
Assert-NotContains $Source 'Backup target already exists' "Existing .default backups can still block Link recreation."
...
if (movedLinkSource !== "C:\\Target\\settings.ini" ||
        movedLinkBackup !== "C:\\Target\\settings.ini.default.2") {
    throw new Error("Existing .default backup still blocked regular-file Link target preservation.");
}
fso = {
