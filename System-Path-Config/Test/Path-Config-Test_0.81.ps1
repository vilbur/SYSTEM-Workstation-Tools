...
Assert-Contains $Source 'var VERSION = "0\.81";' "HTA internal version is not 0.81."
...
if ($Source.IndexOf('value = trimText(value).replace(/\//g, "\\").replace(/\\+/g, "\\");') -lt 0) { $Failures.Add("Config-mode paths are not converted to native single backslashes.") }
