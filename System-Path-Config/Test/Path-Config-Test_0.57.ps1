...
Assert-Contains $Source 'var VERSION = "0\.57";' "HTA internal version is not 0.57."
...
if ([regex]::Matches($Source, '<col style="width:56px"><col style="width:56px"><col style="width:56px"><col style="width:40px">').Count -lt 2) {
    $Failures.Add("Path tables do not reserve spaced ADMIN, STARTUP, MENU, and right-edge columns.")
}
...
Assert-Contains $Source '/t REG_BINARY /d " \+ STARTUP_APPROVED_ENABLED \+ " /f"' "Startup Apps status is not written as REG_BINARY."
...
if ([regex]::Matches($Source, 'writeStartupApproval\(startupName, result,').Count -lt 2) {
    $Failures.Add("Fixed and Programs startup creation do not both enable Windows Startup Apps status.")
}
