...
Assert-Contains $Source 'var VERSION = "0\.84";' "HTA internal version is not 0.84."
...
Assert-Contains $Source 'assertSafeLinkPaths\(src, targetPath, isDirectory\);\s*prepareLinkTarget\(targetPath\);' "Link paths are mutated before the source-safety guard."
Assert-Contains $Source 'assertSafeLinkPaths\(src, shortcutPath, fso[.]FolderExists\(src\)\);\s*if \(fso[.]FileExists\(shortcutPath\)\)' "Shortcut destinations are mutated before the source-safety guard."
...
if (runMklink('mklink "C:\\Target" "D:\\Source"') !== 0 || linkTestCommands.length !== 2 ||
    ...
    throw new Error("mklink privilege failure was not retried through hidden UAC elevation.");
}
