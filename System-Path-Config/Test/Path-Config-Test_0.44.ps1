...
Assert-Contains $Source 'var VERSION = "0\.44";' "HTA internal version is not 0.44."
...
Assert-Contains $Source '\.button\[disabled\],[\s\S]*?cursor: default;' "Disabled Save State button styling is missing."
...
Assert-Contains $Source 'if \(state\.mode === "Config"\) \{[\s\S]*?<th>Target name</th>[\s\S]*?\} else \{[\s\S]*?class="data-table program-table links-apply-table"[\s\S]*?<th>Source</th><th>Target</th><th>Type</th>' "Links does not use the compact Source/Target/Type table only in APPLY mode."
Assert-Contains $Source 'readonlyCell\(row\.src, "pathcheck"\) \+ readonlyCell\(target, ""\) \+ readonlyCell\(row\.type, ""\)' "Links APPLY rows do not contain exactly Source, combined Target, and Type values."
...
Assert-Contains $Source '"Select target folder", initialPath\)' "Link-target Browse does not pass its current field path."
