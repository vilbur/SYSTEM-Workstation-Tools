...
Assert-Contains $Source 'var VERSION = "0\.38";' "HTA internal version is not 0.38."
...
Assert-Contains $Source '\$dialog\.AutoUpgradeEnabled=\$true' "Folder picker does not explicitly request the upgraded native Common Item Dialog."
...
if (pickedPath !== "E:\\Data" || lastBrowseDirectory !== "E:\\Data" ||
        ...
        pickerScript.indexOf("$dialog.AutoUpgradeEnabled=$true;") < 0 ||
