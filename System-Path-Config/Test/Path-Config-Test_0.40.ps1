...
Assert-Contains $Source 'var VERSION = "0\.40";' "HTA internal version is not 0.40."
...
Assert-NotContains $Source '<th>Run as admin</th>|<th>Run on startup</th>' "Legacy Run as/Run on header prefixes remain."
if ([regex]::Matches($Source, '<th>Admin</th>').Count -lt 2) {
    $Failures.Add("Admin header is not used in both path tables.")
}
if ([regex]::Matches($Source, '<th>Startup</th>').Count -lt 2) {
    $Failures.Add("Startup header is not used in both path tables.")
}
Assert-Contains $Source '\(expected \? "Yes" : "No"\)' "Administrator status does not display the configured checkbox state."
Assert-Contains $Source 'function yesNoCell\(value\)[\s\S]*?\(value \? "Yes" : "No"\)' "Boolean Apply status is not displayed as full Yes/No text."
Assert-Contains $Source 'var out = renderProgramPaths\(tabIndex, data\.paths\);\s*out \+= renderLinksSection\(tabIndex, data\.links\);\s*out \+= renderNameValueSection\(tabIndex, "envs", "Environment Variables", data\.envs, false\);\s*out \+= renderExecSection\(tabIndex, data\.execs\);' "Program sections are not ordered Paths, Links, Environment Variables, Executables."
Assert-NotContains $Source 'No entries configured\.|No executables configured\.|No links configured\.' "Visible empty-state labels remain."
if ([regex]::Matches($Source, '<div class="empty-state">&nbsp;</div>').Count -lt 3) {
    $Failures.Add("Blank empty-state placeholders do not preserve all three section heights.")
}
Assert-Contains $Source '<div class="section-head"><span class="section-title">[^<]+</span>[^<]*<button class="section-action"' "+ Add buttons are not placed beside section titles."
...
Assert-Contains $Source '\.section-action\s*\{\s*display: inline-block;\s*margin-left: 10px;\s*vertical-align: 2px;' "Tab section buttons are not aligned beside labels on the left."
...
Assert-Contains $Source 'PathConfig\.NativeFolderDialog\]::Show\([\s\S]*?powerShellLiteral\(currentBrowseDirectory\(\)\)' "Native folder picker does not use the remembered directory."
...
if (pickedPath !== "E:\\Data" || lastBrowseDirectory !== "E:\\Data" ||
        ...
        pickerScript.indexOf("'Select target folder'") < 0) {
