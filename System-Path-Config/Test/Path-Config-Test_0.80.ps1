...
Assert-Contains $Source 'var VERSION = "0\.80";' "HTA internal version is not 0.80."
...
Assert-Contains $Source 'value = trimText\(value\)\.replace\(/\\\\/g, "/"\)' "Config-mode paths are not converted to forward slashes."
Assert-Contains $Source 'expanded && fso\.FolderExists\(expanded\) && !/\\/\$/.test\(value\)' "Existing Config-mode directories do not receive a missing trailing slash."
Assert-Contains $Source 'path = path\.charAt\(0\)\.toUpperCase\(\) \+ path\.substr\(1\)' "Browsed drive letters are not capitalized."
Assert-Contains $Source 'while \(/\\/\$/.test\(path\)\)[\s\S]*?path \+= "/";' "Browsed folders do not end in exactly one trailing slash."
...
if (configPathElement.value !== "C:/Users/vilbur/.gk" || state.persistent[0].path !== "C:/Users/vilbur/.gk") {
    throw new Error("Config-mode focus loss did not safely normalize a dotted path to forward slashes.");
}
