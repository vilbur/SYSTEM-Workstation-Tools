...
Assert-Contains $Source 'var VERSION = "0\.83";' "HTA internal version is not 0.83."
...
Assert-Contains $Source 'function removeBrokenReparsePoint\(path\)' "Broken destination reparse-point cleanup is missing."
Assert-Contains $Source '\$item\.Attributes -band \[IO\.FileAttributes\]::ReparsePoint' "Broken-link cleanup does not require an actual reparse point."
Assert-Contains $Source 'if\(Test-Path -LiteralPath \$resolved\)\{\$broken=\$false;break\}' "Broken-link cleanup can remove a link whose target still exists."
...
Assert-Contains $Source 'else if [(]backupExists[)] [{]\s*fso[.]DeleteFolder[(]path, true[)];' "A regular folder target is not removed when its canonical .default backup already exists."
...
fso = {
    FileExists: function() { throw new Error("Broken reparse cleanup did not return before FileExists."); },
    FolderExists: function() { throw new Error("Broken reparse cleanup did not return before FolderExists."); }
};
prepareLinkTarget("C:\\BrokenLink");
if (linkTestCommands.length !== 1 || linkTestCommands[0].command.indexOf("Get-Item -LiteralPath") < 0 ||
        linkTestCommands[0].command.indexOf("Remove-Item -LiteralPath") < 0 ||
        linkTestCommands[0].style !== 0 || linkTestCommands[0].wait !== true) {
    throw new Error("Broken reparse-point cleanup was not run safely and hidden.");
}
linkTestCommands = [];
wsh = {
    Run: function(command, style, wait) {
