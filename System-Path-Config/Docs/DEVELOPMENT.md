...
The latest approved stable version is `0.70`. The root `STABLE_VERSION` marker remains `0.70` until the user explicitly promotes a newer release. The current development source is `Path-Config.hta` version `0.88`.
...
The next code delivery must be version `0.89` unless another version has already been approved in the repository.
