...
sendVideoToVlc(video_path)
{
    ...
    fullscreen_result := enterVlcFullscreenOnMonitor(new_vlc_window_id, target_monitor_index, 4)

    if (!fullscreen_result)
    {
        Sleep, 1000
        fullscreen_result := enterVlcFullscreenOnMonitor(new_vlc_window_id, target_monitor_index, 2)
    }
...
*/
/*
Return the monitor whose complete bounds are covered by a window.
*/
getWindowFullscreenMonitorIndex(window_id, tolerance := 12)
{
    ...
    Loop, %monitor_count%
    {
        ...
        monitor_width := monitor_areaRight - monitor_areaLeft
        monitor_height := monitor_areaBottom - monitor_areaTop

        if (Abs(window_x - monitor_areaLeft) <= tolerance
            && Abs(window_y - monitor_areaTop) <= tolerance
...
/*
Wait until VLC is confirmed fullscreen on the target monitor.
*/
waitForVlcFullscreen(window_id, monitor_index, timeout_ms := 3000)
{
    started_at := A_TickCount

    while (A_TickCount - started_at < timeout_ms)
    {
        fullscreen_monitor := getWindowFullscreenMonitorIndex(window_id)

        if (fullscreen_monitor > 0
            && (monitor_index < 1 || fullscreen_monitor = monitor_index))
        {
            return true
        }
...
/*
Wait until VLC has left fullscreen so its window can be moved.
*/
waitForVlcWindowed(window_id, timeout_ms := 2000)
{
    started_at := A_TickCount

    while (A_TickCount - started_at < timeout_ms)
    {
        if (getWindowFullscreenMonitorIndex(window_id) = 0)
        {
            return true
        }
...
/*
Send VLC's video-fullscreen shortcut directly to VLC.
*/
sendVlcFullscreenKey(window_id)
{
    if (window_id = "" || !WinExist("ahk_id " . window_id))
    {
        return false
    ...
    ControlSend,, f, ahk_id %window_id%
    return true
}

/*
Send VLC's fullscreen-interface shortcut as a fallback.
*/
sendVlcFullscreenInterfaceKey(window_id)
{
    if (window_id = "" || !WinExist("ahk_id " . window_id))
    {
        return false
    ...
    ControlSend,, {F11}, ahk_id %window_id%
    return true
}
...
enterVlcFullscreenOnMonitor(window_id, monitor_index, retry_count := 3)
{
    ...
    if (waitForVlcFullscreen(window_id, monitor_index, 1800))
    {
        return true
    }
    ...
    Loop, %attempts%
    {
        ...
        if (fullscreen_monitor > 0
            && monitor_index > 0
            && fullscreen_monitor != monitor_index)
        {
            sendVlcFullscreenKey(window_id)
            waitForVlcWindowed(window_id, 2200)
        }

        if (monitor_index > 0)
        {
            moveWindowToMonitorWithRetries(window_id, monitor_index, 4)
        }
        ...
        if (waitForVlcFullscreen(window_id, monitor_index, 3000))
        {
            return true
        }
        ...
        {
            sendVlcFullscreenKey(window_id)
            waitForVlcWindowed(window_id, 1800)
        }
    }
